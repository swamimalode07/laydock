...
export const Item=({
    title,onClick,icon:Icon,id,active,documentIcon,isSearch,level=0,onExpand,expanded
}:ItemProps)=>{ 
    const {user}=useUser()
    ...
    const onCreate=(e:React.MouseEvent<HTMLDivElement,MouseEvent>)=>{
        ...
        toast.promise(promise,{
            loading:"Creating a new subpage",
            success:"New subpage created",
            error:"Failed to create new suboage"
        })
    }
    ...
    className={cn(
        "group min-h-[27px] py-1 pr-3 w-full hover:bg-primary/5 flex items-center text-muted-foreground",
